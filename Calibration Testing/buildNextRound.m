...
censusPath = 'C:/Users/Jack/OneDrive - University of East Anglia/ISF - Migration modelling consultancy/Datasets/Census migration/';
...
%catch
    ...
    for indexI = 1:length(fileList)
        try
            ...
            %% --- Agricultural participation (PRIMARY scored target) ---
            % SCORED target: national population-weighted fraction of agent
            % layer-participations that are in an agricultural layer
            ...
            if isfield(currentRun.output, 'countAgentsPerLayer')
                ...
                if nL == nRegions && nLay >= max([urbanLayerIdx agLayerIdx])
                    ...
                    agFracRun    = agCount    ./ totalCount;

                    % Per-region urban diagnostics (no longer scored)
                    ...
                    % National pop-weighted urban fraction (diagnostic)
                    urbanFrac_nat_run = sum(urbanFracRun .* popData) / sum(popData);
                    urbanFracNatError = (urbanFrac_nat_run - urbanFrac_nat_data)^2;

                    % National pop-weighted ag fraction (SCORED target)
                    agFrac_nat_run = sum(agFracRun .* popData) / sum(popData);
                    agFracNatError = (agFrac_nat_run - agFrac_nat_data)^2;
                end
            end
...
if height(outputListRun) > 0
    ...
    validAgNat = outputListRun.agFracNatError(~isnan(outputListRun.agFracNatError));
    ...
    % Demoted urban-residence target -- now diagnostic only (was scored in rounds <8)
    validUrbanNat = outputListRun.urbanFrac_nat_run(~isnan(outputListRun.urbanFrac_nat_run));
    if ~isempty(validUrbanNat)
        fprintf('urbanFrac_nat_run        %7.4f  %7.4f  %7.4f   (former target 0.19, now diagnostic)\n', ...
            mean(validUrbanNat), median(validUrbanNat), max(validUrbanNat));
    end
end
...
% Unlike ag/FI this is a goodness metric (higher = better), so we divide
...
% Normalise ag-fraction error to 0-1 score (1 = perfect, 0 = worst).
% Scored target switched from urban-residence to ag-participation in
% round 8: see comments in the per-run scoring block above for rationale.
maxAgError = max(outputListRun.agFracNatError(hasAg));
if isempty(maxAgError) || maxAgError == 0; maxAgError = 1; end
agScore = 1 - outputListRun.agFracNatError / maxAgError;
agScore(~hasAg) = NaN;
...
activeWeights = migWeight + agWeight * any(hasAg) + fiWeight * any(hasFI);
combinedScore = (migWeight / activeWeights) * migScore;

if any(hasAg)
    agScoreAdj = agScore;
    agScoreAdj(~hasAg) = 0;
    combinedScore = combinedScore + (agWeight / activeWeights) * agScoreAdj;
end
if any(hasFI)
    fiScoreAdj = fiScore;
    fiScoreAdj(~hasFI) = 0;
...
fprintf('Combined score weights: migration=%.0f%%, ag-participation=%.0f%%, food insecurity=%.0f%%\n', ...
    migWeight/activeWeights*100, ...
    agWeight*any(hasAg)/activeWeights*100, ...
...
expList = dir('../Outputs/experiment_*.mat');
if isempty(expList)
